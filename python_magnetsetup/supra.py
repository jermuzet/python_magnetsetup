...
def Supra_setup(MyEnv, mname: str, confdata: dict, cad: Supra, method_data: List, templates: dict, current: float=31.e+3, debug: bool=False):
    ...
    if cad.detail is None:
        ...
        if cad.detail == 'dblpancake':
            ...
            part_eletric = [name for name in snames if re.search(search_pattern, name)] # find all dblpancake in
        elif cad.detail == 'pancake':
            search_pattern  = f'{name}_{cad.name}_dp_\\d_p[01]'
            part_electric = [name for name in snames if re.search(search_pattern, name)] # find all pancake
        elif cad.detail == 'tape':
            part_electric = [name for name in snames if name.endswith('_SC')] # find all _SC in snames
        else:
            raise RuntimeError(f'Supra_Setup: {cad.name} - cad.detail unsupported value {cad.detail}- expected dblpancake|pancake|tape')
    ...
    for physic in templates['physic']:
        mmodels[physic] = create_models_supra(gdata, confdata, templates, method_data, physic, debug)
